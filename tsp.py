...
cost_matrix = np.random.randint(1, max_cost,
                                size=(num_nodes, num_nodes))  # this will generate a random integer cost matrix
...
print('Cost of tour found by the solver is {} and cost computed by us for verification is {} '.format(
    round(prob.value, 2), round(verified_cost, 2)))
print('Time taken to solve the problem instance after formulation is {} seconds'.format(
    round(prob.solver_stats.solve_time, 3)))
